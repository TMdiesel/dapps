...
const TransactionHistory: React.FC = () => {
  ...
  useEffect(() => {
    console.log(
      "TransactionHistory useEffect triggered with smartAccount:",
      smartAccount
    );
    const loadTransactionHistory = async () => {
      ...
      try {
        setLoading(true);
        console.log(
          "Loading transaction history for smartAccount:",
        ...
        const history = await aaProvider.getTransactionHistory(
          fullSmartAccount!
        );
  ...
  const determineTransactionType = (tx: any): string => {
    if (tx.data && tx.data !== "0x") {
      ...
      if (data.includes("a9059cbb")) return "token"; // transfer function signature
      if (data.includes("40c10f19")) return "nft"; // mint function signature
      if (data.includes("38ed1739")) return "dex"; // swapExactTokensForTokens signature
      if (data.includes("b61d27f6")) return "execute"; // Smart Account execute
    }

    // Check transaction description for additional context
    if (tx.description) {
      const desc = tx.description.toLowerCase();
      if (desc.includes("nft") || desc.includes("mint")) return "nft";
      if (desc.includes("token") || desc.includes("transfer")) return "token";
      if (desc.includes("swap") || desc.includes("dex")) return "dex";
    }
  ...
  const getTransactionAction = (tx: any): string => {
    // Use description from aa-provider if available
    if (tx.description && tx.description !== "Transaction") {
  ...
  const formatTransactionAmount = (tx: any): string => {
    const type = determineTransactionType(tx);
    const ethAmount = parseFloat(tx.value || "0");
    ...
    switch (type) {
      ...
      case "dex":
        return `Token Swap`;
      case "execute":
        // For execute transactions, check the actual operation
        if (tx.description?.includes("NFT")) return "1 NFT";
        if (tx.description?.includes("Token")) return `Token Transfer`;
        ...
        {transactions.length === 0 ? (
          <div className="text-center py-8 text-gray-500">
            ...
            <p className="text-sm mt-1">
              {smartAccount
                ? "Make your first transaction to see history here"
